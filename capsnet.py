...
from capsule_layer_new import ConvCapsuleLayer, Mask, Length, DeconvCapsuleLayer

def CapsNetBasic(input_shape, n_class=2):
    x = layers.Input(shape=input_shape)

    # Layer 1: Just a conventional Conv2D layer
    conv1 = layers.Conv2D(filters=512, kernel_size=5, strides=1, padding='same', activation='relu', name='conv1')(x)
    conv2 = layers.Conv2D(filters=256, kernel_size=5, strides=1, padding='same', activation='relu', name='conv2')(conv1)
    # The shape of conv1 is [batch, 512, 512, 256]
    # Reshape layer to be 1 capsule x [filters] 256 atoms
    _, H, W, C = conv2.get_shape()
    conv2_reshaped = layers.Reshape((H.value, W.value, 1, C.value))(conv2)
    ...
    primary_caps = ConvCapsuleLayer(kernel_size=5, num_capsule=8, num_atoms=32, strides=1, padding='same',
                                    routings=1, name='primarycaps')(conv2_reshaped)
