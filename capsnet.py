...
from capsule_layers_new import ConvCapsuleLayer, Mask, Length, DeconvCapsuleLayer
...
def DevSegCaps(input_shape, n_class=2):
    x = layers.Input(shape=input_shape)

    # Layer 1: Just a conventional Conv2D layer
    conv1 = layers.Conv2D(filters=128, kernel_size=5, strides=1, padding='same', activation='relu', name='conv1')(x)
    ...
    primary_caps = ConvCapsuleLayer(kernel_size=5, num_capsule=2, num_atoms=32, strides=1, padding='same',
                                    routings=1, name='primarycaps')(conv1_reshaped)
    ...
    third_caps = ConvCapsuleLayer(kernel_size=5, num_capsule=8, num_atoms=8, strides=1, padding='same',
                                  routings=2, name='thirdcaps')(secondary_caps)

    forth_caps = ConvCapsuleLayer(kernel_size=5, num_capsule=4, num_atoms=4, strides=1, padding='same',
                                  routings=2, name='forthcaps')(third_caps)

    fifth_caps = ConvCapsuleLayer(kernel_size=5, num_capsule=2, num_atoms=8, strides=1, padding='same',
                                  routings=2, name='fifthcaps')(forth_caps)

    # Layer 4: Convolutional Capsule: 1x1
    seg_caps = ConvCapsuleLayer(kernel_size=1, num_capsule=1, num_atoms=16, strides=1, padding='same',
                                routings=3, name='seg_caps')(fifth_caps)

    # Layer 4: This is an auxiliary layer to replace each capsule with its length. Just to match the true label's shape.
    out_seg_1 = Length(num_classes=n_class, seg=True, name='out_seg_1')(seg_caps)

    out_seg = layers.Conv2D(filters=1, kernel_size=1, strides=1, padding='same',
                            activation='sigmoid', name='out_seg')(out_seg_1)
