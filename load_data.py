    ...
    for folder in get_sub_folder(data_dir):
        ...
        for sub_folder in get_sub_folder(os.path.join(data_dir, folder)):
            ...
            folder_dict = {"training_28": [6, 3], "training_30": [4, 5], "training_31": [6, 3], "training_32": [4, 5],
                           "training_33": [5, 4], "training_35": [6, 3], "training_8": [2, 1], "training_6": [3, 0],
                           "training_13": [3, 0], "training_14": [3, 0], "training_16": [3, 0], "training_19": [3, 0],
                           "training_22": [3, 0], "training_23": [3, 0], "training_24": [3, 0], "training_26": [3, 0]}
...
def generate_test_batches(test_images, net_input_shape, batchSize=1, numSlices=1, subSampAmt=0,
    ...
    if numSlices == 1:
        ...
        np.random.seed(None)
        subSampAmt = int(rand(1) * (19 * 0.05))

    while True:
        count = 0
        indexes = np.arange(0, 19, stride)

        for j in indexes:
            if img_batch.ndim == 4:
                img_batch[count, :, :, :] = test_images[0][:, :, j:j + numSlices * (subSampAmt + 1):subSampAmt + 1]
            elif img_batch.ndim == 5:
                # Assumes img and mask are single channel. Replace 0 with : if multi-channel.
                img_batch[count, :, :, :, 0] = test_images[0][:, :, j:j + numSlices * (subSampAmt + 1):subSampAmt + 1]
            else:
