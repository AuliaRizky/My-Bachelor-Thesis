...
def augmentImages(batch_of_images, batch_of_masks):
    for i in range(len(batch_of_images)):
        ...
        if np.random.randint(0, 10) == 7:
            batch_of_images = np.flipud(batch_of_images)
            batch_of_masks = np.flipud(batch_of_masks)

        # Apply Gaussian Filter
        if np.random.randint(0, 10) == 7:
            batch_of_images = gaussian_filter(batch_of_images, sigma=0.1)

        # Sharpening The Image
        if np.random.randint(0, 10) == 3:
            blurred_batch = gaussian_filter(batch_of_images, sigma=0.1)
            filter_blurred_batch = gaussian_filter(blurred_batch, sigma=0.075)
            batch_of_images = blurred_batch + 30 * (blurred_batch - filter_blurred_batch)
