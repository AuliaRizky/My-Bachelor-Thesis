...
# from load_data_kfold import generate_train_batches, generate_val_batches
...
def get_callbacks(arguments):
    ...
    csv_logger = CSVLogger(('D:\Engineering Physics\Skripsi\Program\Ischemic Stroke Segmentation\logs\BaseCaps.csv'), separator=',')
    tb = TensorBoard(arguments.tf_log_dir, batch_size=arguments.batch_size, histogram_freq=0)
    model_checkpoint = ModelCheckpoint(join(arguments.check_dir, 'SegCapsBasic' + '_model_' + arguments.time + '.hdf5'),
...
def train(args, images_train, g_t_train, u_model, net_input_shape):
    ...
    # history = model.fit(x=images_train, y=g_t_train, batch_size=1, epochs=2, verbose=1, callbacks = callbacks, validation_data = images_val, shuffle = True )

    history = model.fit_generator(generate_train_batches(images_train, g_t_train, net_input_shape, net='segcapsbasic', batchSize=args.batch_size, numSlices=args.slices, subSampAmt=args.subsamp,stride=args.stride, shuff=args.shuffle_data, aug_data=args.aug_data),
                                  steps_per_epoch=1/args.batch_size, epochs=5, verbose=1, callbacks=callbacks,
                                  validation_data=generate_val_batches(images_train, g_t_train, net_input_shape, net='segcapsbasic',batchSize=args.batch_size,  numSlices=args.slices, subSampAmt=0,stride=5, shuff=args.shuffle_data),
                                  validation_steps=1/args.batch_size, max_queue_size=10, workers=4, use_multiprocessing=False)
