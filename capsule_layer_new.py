...
class ConvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        # votes reshaped to shape [None, 1, 192, 192, 8, 32]
...
def update_routing(votes, biases, logit_shape, num_dims, input_dim, output_dim,
    ...
    # Votes shape [None, 1, 192, 192, 8, 32]
    # Reshape it to [32, None, 192, 192, 8]
    ...
    def _body(i, logits, activations):
        ...
        # routing coefficient (denoted as r_ti_x|y) is c_ti in layer l the product of b_ij softmax
        route = tf.nn.softmax(logits, axis=-1)
        preactivate_unrolled = route * votes_trans
        # Transform the shape to [1, 192, 192, 8, None]
        preact_trans = tf.transpose(preactivate_unrolled, r_t_shape)
        # Preactivate is p_xy, apply reduce sum for 8 dim
        preactivate = tf.reduce_sum(preact_trans, axis=1) + biases
        # Squash the p_xy to produce v_xy eq. 2
    ...
    # Make a logits with shape = logit_shape filled with 0
    # The shape of logits is [None, 1, 192, 192, 8]
    logits = tf.fill(logit_shape, 0.0)

    # i is the index for how many routing
